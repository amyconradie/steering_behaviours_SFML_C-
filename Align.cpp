...
void Align::init(sf::RenderWindow* _window)
{
	this->window = _window;
...
void Align::CreateAgent(sf::Vector2f _spawnPoint)
{
...
void Align::update(float dtime, sf::Event* event)
{
	switch (event->type) {
...
const char* Align::getTitle() { return "Behaviour: Alignment - Wander"; }


// Steering Behaviours

sf::Vector2f Align::SteeringBehaviour(Agent* agent)
{
	float radius = RadiusOfCircumCircle(agent->getSize().x, agent->getSize().y);
	float neighbourDistance = radius;
...
bool Align::lineIntersectsAgent(sf::Vector2f ahead, sf::Vector2f ahead2, Agent* obstacle)
{
...
float Align::Distance(sf::Vector2f a, sf::Vector2f b)
{
	return sqrt((a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y));
}

float Align::RadiusOfCircumCircle(float w, float h)
{
...
sf::Vector2f Align::Truncate(sf::Vector2f v, float max)
{
...
float Align::OppositeFloat(float f) {
	float opposite;
...
float Align::Magnitude(sf::Vector2f vector)
{
	return (float)sqrt(vector.x * vector.x + vector.y * vector.y);
}

sf::Vector2f Align::Normalize(sf::Vector2f vector)
{
...
float Align::RandomRange(int min, int max)
{
	int n = max - min + 1;
