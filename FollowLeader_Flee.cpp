...
FollowLeader_Flee::FollowLeader_Flee()
{
	this->window = nullptr;
}
...
void FollowLeader_Flee::init(sf::RenderWindow* _window)
{
	this->window = _window;
...
void FollowLeader_Flee::CreateAgent(sf::Vector2f _spawnPoint)
{
...
void FollowLeader_Flee::update(float dtime, sf::Event* event)
{
	switch (event->type) {
	...
	if (!agents.empty())
	{
		...
		for (auto& agent : agents)
		{
			...
			if (leader == nullptr) // if first item of type to spawn
			{
				...
				sf::Vector2f steering_force = Flee(agent);
...
const char* FollowLeader_Flee::getTitle() { return "Behaviour: Follow the Leader - Flee"; }

sf::Vector2f FollowLeader_Flee::SteeringBehaviour(Agent* agent, Agent* leader)
{
...
bool FollowLeader_Flee::isOnLeaderSight(Agent* agent, Agent* leader, sf::Vector2f leaderAhead, float LEADER_SIGHT_RADIUS)
{
...
sf::Vector2f FollowLeader_Flee::Separate(Agent* agent, float desiredSeparation)
{
...
sf::Vector2f FollowLeader_Flee::Arrive(Agent* agent, float slowingRadius)
{
...
sf::Vector2f FollowLeader_Flee::Evade(Agent* prey, Agent* predator)
{
...
sf::Vector2f FollowLeader_Flee::Flee(Agent* agent)
{
	// check that agent wont constantly back into the walls to get away from the target
	if (agent->BorderRule()) {
		*agent->getTarget() = *agent->getPosition();
	...
	sf::Vector2f desiredVelocity = *agent->getTarget()  - *agent->getPosition(); // move away from target
	desiredVelocity = Normalize(desiredVelocity);
...
bool FollowLeader_Flee::lineIntersectsAgent(sf::Vector2f ahead, sf::Vector2f ahead2, Agent* obstacle)
{
...
float FollowLeader_Flee::Distance(sf::Vector2f a, sf::Vector2f b)
{
	return sqrt((a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y));
}

float FollowLeader_Flee::RadiusOfCircumCircle(float w, float h)
{
...
float FollowLeader_Flee::Magnitude(sf::Vector2f vector)
{
	return (float)sqrt(vector.x * vector.x + vector.y * vector.y);
}

sf::Vector2f FollowLeader_Flee::Normalize(sf::Vector2f vector)
{
...
float FollowLeader_Flee::RandomRange(int min, int max)
{
	int n = max - min + 1;
