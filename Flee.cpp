...
void Flee::init(sf::RenderWindow* _window)
{
...
void Flee::update(float dtime, sf::Event* event)
{
	switch (event->type) {
...
const char* Flee::getTitle() { return "Behaviour: Flee"; }


// Steering Behaviours

sf::Vector2f Flee::SteeringBehaviour(Agent* agent)
{
...
float Flee::OppositeFloat(float f) {
	float opposite;
...
float Flee::Magnitude(sf::Vector2f vector)
{
	return (float)sqrt(vector.x * vector.x + vector.y * vector.y);
}

sf::Vector2f Flee::Normalize(sf::Vector2f vector)
{
...
sf::Vector2f Flee::DesiredVelocity(sf::Vector2f current_position, sf::Vector2f target_position, float max_velocity)
{
...
sf::Vector2f Flee::SteeringForce(sf::Vector2f desiredVelocity, sf::Vector2f velocity, float max_velocity, float max_force)
{
...
float Flee::RandomRange(int min, int max)
{
	int n = max - min + 1;
