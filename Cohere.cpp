...
void Cohere::init(sf::RenderWindow* _window)
{
	this->window = _window;
...
void Cohere::CreateAgent(sf::Vector2f _spawnPoint)
{
...
void Cohere::update(float dtime, sf::Event* event)
{
	switch (event->type) {
...
const char* Cohere::getTitle() { return "Behaviour: Cohesion - Wander"; }


// Steering Behaviours

sf::Vector2f Cohere::SteeringBehaviour(Agent* agent)
{
	float radius = RadiusOfCircumCircle(agent->getSize().x, agent->getSize().y);
	float neighbourDistance = radius * 2.f ;
...
bool Cohere::lineIntersectsAgent(sf::Vector2f ahead, sf::Vector2f ahead2, Agent* obstacle)
{
...
float Cohere::Distance(sf::Vector2f a, sf::Vector2f b)
{
	return sqrt((a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y));
}

float Cohere::RadiusOfCircumCircle(float w, float h)
{
...
sf::Vector2f Cohere::Truncate(sf::Vector2f v, float max)
{
...
float Cohere::OppositeFloat(float f) {
	float opposite;
...
float Cohere::Magnitude(sf::Vector2f vector)
{
	return (float)sqrt(vector.x * vector.x + vector.y * vector.y);
}

sf::Vector2f Cohere::Normalize(sf::Vector2f vector)
{
...
float Cohere::RandomRange(int min, int max)
{
	int n = max - min + 1;
