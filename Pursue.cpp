...
void Pursue::init(sf::RenderWindow* _window)
{
	this->window = _window;
	...
	otherAgent->setPosition(otherAgent->getRandomPosition(otherAgent->getTargetArea()));
	otherAgent->setMass(0.02f);
...
void Pursue::update(float dtime, sf::Event* event)
{
	switch (event->type) {
...
const char* Pursue::getTitle() { return "Behaviour: Pursue"; }

// Steering Behaviours

sf::Vector2f Pursue::SteeringBehaviour(Agent* predator, Agent* prey)
{
	sf::Vector2f distance = *prey->getPosition() - *predator->getPosition(); // distance between prey and agent
	float updatesAhead = Magnitude(distance) / predator->getMaxVelocity(); // how many updates the target needs to move from its current position to the pursuer position
	sf::Vector2f futurePosition = *prey->getPosition() + prey->getVelocity() * updatesAhead;

	predator->setTarget(futurePosition);

	return Seek(predator);
}


sf::Vector2f Pursue::Seek(Agent* agent)
{
...
float Pursue::OppositeFloat(float f) {
	float opposite;
...
float Pursue::Magnitude(sf::Vector2f vector)
{
	return (float)sqrt(vector.x * vector.x + vector.y * vector.y);
}

sf::Vector2f Pursue::Normalize(sf::Vector2f vector)
{
...
sf::Vector2f Pursue::DesiredVelocity(sf::Vector2f current_position, sf::Vector2f target_position, float max_velocity)
{
...
sf::Vector2f Pursue::SteeringForce(sf::Vector2f desiredVelocity, sf::Vector2f velocity, float max_velocity, float max_force)
{
...
float Pursue::RandomRange(int min, int max)
{
	int n = max - min + 1;
